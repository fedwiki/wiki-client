...
export function textEditor ($item, item, option) {
  ...
  const focusoutHandler = function() {
    ...
    item[option.field||'text'] = $textarea.val();
    if ( item[option.field||'text'] ) {
      ...
      plugin.doPlugin($item.empty(), item);
      if (option.after) {
  ...
  var $textarea = $(`<textarea>${escape(original)}${escape(option.suffix != null ? option.suffix : '')}</textarea>`)
...
var spawnEditor = function($page, $before, type, text) {
  ...
  textEditor($item, item, {after: (before != null ? before.id : undefined)});
};
