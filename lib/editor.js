...
var textEditor = function($item, item, option) {
  ...
  const keydownHandler = function(e) {
    ...
    if ((item.type === 'paragraph') || (item.type === 'markdown')) {
      ...
      if (e.which === $.ui.keyCode.ENTER) {
        ...
        if (item.type === 'markdown') { return enterCount = 0; }
      }
    }
  };
  ...
  $item.off();
  var original = (left = item[option.field||'text']) != null ? left : '';
...
var spawnEditor = function($page, $before, type, text) {
  ...
  return textEditor($item, item, {after: (before != null ? before.id : undefined)});
};
