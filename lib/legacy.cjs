...
const {bind:target_bind} = require('./target.mjs');
const {info} = require('./license.mjs');
  ...
  $('.main')
    .sortable({handle: '.page-handle', cursor: 'grabbing'})
      ...
      .on('click', '.show-page-license', function(e) {
        ...
        dialog_open(`License for ${title}`, info($page));
      })
      .on('click', '.show-page-source', function(e) {
        e.preventDefault();
        const $page = $(this).parents('.page');
        const page = lineup.atKey($page.data('key')).getRawPage();
        dialog_open(`JSON for ${page.title}`,  $('<pre/>').text(JSON.stringify(page, null, 2)));
      })
      ...
      .on('dblclick', '.revision', function(e) {
        ...
        dialog_open(`Revision ${rev}, ${action.type} action`, $('<pre/>').text(json));
      })
