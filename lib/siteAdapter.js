...
, findQueueWorkers); // start with just 8 process working on the queue
...
// example use wiki.site('fed.wiki').get('welcome-visitors')
...
//        findAdapterQ.push {site: site}, (prefix) ->
    ...
    refresh(done) {
      ...
      routeStore.removeItem(site).then(() => findAdapterQ.push({site}, prefix => routeStore.setItem(site, prefix).then(function(value) {
        if (prefix === "") {
