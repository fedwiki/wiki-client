...
routeStore.iterate(function(value, key ) {
...
withCredsStore.iterate(function(value, key ) {
...
const findAdapter = (site, done) => routeStore.getItem(site).then(function(value) {
  ...
  if ((value == null)) {
    return findAdapterQ.push({site}, function(prefix) {
      ...
      return routeStore.setItem(site, prefix).then(() => done(prefix)).catch(function(err) {
...
export const local = {
  ...
  get(route, callback) {
    ...
    const page= localStorage.getItem(route.replace(/\.json$/,''));
    if (page ) {
...
export const site = function(site) {
  if (!site || (site === window.location.host)) { return origin; }
  if (site === 'recycler') { return recycler; }
...
//        findAdapterQ.push {site: site}, (prefix) ->
    ...
    refresh(done) {
      ...
      return routeStore.removeItem(site).then(() => findAdapterQ.push({site}, prefix => routeStore.setItem(site, prefix).then(function() {
        if (prefix === "") {
