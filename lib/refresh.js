...
const handleDrop = function(evt, ui, originalIndex, originalOrder) {
  ...
  pageHandler.put($destinationPage,
                  {id: item.id, type: 'add', item, after: (before != null ? before.id : undefined), attribution: dragAttribution });
  $('.shadow-copy').remove();
  $item.empty();
  $before.after($item);
  index = $(".item").index($item);
...
var createFactory = function($page) {
  ...
  const $before = $item.prev('.item');
  const before = getItem($before);
  return pageHandler.put($page, {item, id: item.id, type: "add", after: (before != null ? before.id : undefined)});
};
  ...
  if (Object.keys(backlinks).length > 0) {
    ...
    for (var linkSlug in backlinks) {
      ...
      for (var i = 0; i < backlink.sites.length; i++) {
        ...
        if (i < 10) {
          var joint = __guard__(backlink.sites[i-1], x => x.date) === site.date ? "" : " ";
          flags.unshift(joint);
          flags.unshift(`\
<img class="remote"
...
`);
    ...
    if (twins) { return $page.find('.twins').html(`<p><span>${twins.join(", ")}</span></p>`); }
  }
};
...
const renderPageIntoPageElement = function(pageObject, $page) {
  ...
  $page.append($paper);
  const [$handleParent, $twins, $header, $story, $backlinks, $journal, $footer] = Array.from(['handle-parent', 'twins', 'header', 'story', 'backlinks', 'journal', 'footer'].map(function(className) {
...
const createMissingFlag = function($page, pageObject) {
  if (!pageObject.isRemote()) {
    return $('img.favicon',$page).on('error', () => plugin.get('favicon', favicon => favicon.create()));
  }
};
...
const newFuturePage = function(title, create) {
  ...
  if (hits.length > 0) {
    ...
    for (var hit of Array.from(hits)) { pageObject.addItem(hit); }
  } else {
    pageObject.addItem({
      'type': 'future',
