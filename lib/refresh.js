...
function openPagesFromHost () {
  let lastOpenedFrom = localStorage.getItem('lastOpenedFrom')
  const host = document.location.host
  const home = window.prompt('Open these pages from what origin server:', lastOpenedFrom || '')
  if (!home) return;
  if (home !== lastOpenedFrom) {
    localStorage.setItem('lastOpenedFrom', home)
  }
...
;(function () {
  console.log(this)
  this.openPagesFromHost = openPagesFromHost
})()
...
<a onclick="openPagesFromHost()" href= "#">open from</a> .
