...
import { addToJournal } from './addToJournal.mjs';
import * as actionSymbols from './actionSymbols.mjs';
import * as lineup from './lineup.mjs';
import { Resolve } from './resolve.mjs';
const resolve = new Resolve();
import * as random from './random.mjs';
import * as wiki from './wiki.mjs';
import * as pageModule from './page.mjs';
const {
  newPage
} = pageModule;
const {
...
export function emitTwins ($page) {
  let remoteSite, item;
  ...
  const viewing = editDate(page.journal);
  if (viewing ) {
    let bin;
...
const createMissingFlag = function($page, pageObject) {
  if (!pageObject.isRemote()) {
    $('img.favicon',$page).on('error', () => plugin.getPlugin('favicon', favicon => favicon.create()));
  }
};

export function rebuildPage (pageObject, $page) {
...
export function buildPage (pageObject, $page) {
...
export function newFuturePage (title, create) {
