...
util.formatActionTitle = function (action) {
  ...
  if (action.type == 'fork' && action.title) {
    title += " rename" 
  }
  if (action.date) {
    title += ` ${util.formatElapsedTime(action.date)}`
  }
  if (action.attribution?.page) {
    title += `\nfrom ${action.attribution.page}`
  }
  if (action.removedTo?.page) {
...
util.formatDelay = function (msSinceEpoch) {
  ...
  if ((secs = msecs / 1000) < 2) {
    return `in ${Math.floor(msecs)} milliseconds`
  }
  if ((mins = secs / 60) < 2) {
    return `in ${Math.floor(secs)} seconds`
  }
  if ((hrs = mins / 60) < 2) {
    return `in ${Math.floor(mins)} minutes`
  }
