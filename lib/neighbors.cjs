...
const {formatElapsedTime, formatDelay} = require('./util.mjs');
...
const formatNeighborTitle = function(site) {
  ...
  if (sites[site].lastModified !== 0) {
    title += `Updated ${formatElapsedTime(sites[site].lastModified)}`;
    if ((sites[site].nextCheck - Date.now()) > 0) { title += `, next refresh ${formatDelay(sites[site].nextCheck)}`; }
  }
  return title;
};
