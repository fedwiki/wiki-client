...
const { create:revision_create , apply:revision_apply } = require('./revision.mjs');
const addToJournal = require('./addToJournal.mjs');
const { newPage } = require('./page.mjs');
...
var recursiveGet = function({pageInformation, whenGotten, whenNotGotten, localContext}) {
  ...
  adapter.get(`${slug}.json`, function(err, page) {
    if (!err) {
      ...
      if (rev) { page = revision_create(rev, page); }
...
pageHandler.get = function({whenGotten,whenNotGotten,pageInformation}  ) {
  ...
  if (!pageInformation.site) {
    ...
    if (localPage = pageFromLocalStorage(pageInformation.slug)) {
      if (pageInformation.rev) { localPage = revision_create(pageInformation.rev, localPage); }
