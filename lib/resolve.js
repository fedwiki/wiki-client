...
resolve.escape = escape = string => (string || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
...
resolve.resolveLinks = function (string, sanitize = escape) {
  ...
  const internal = function (match, name) {
    ...
    if (slug.length) {
      return stash(
        `<a class="${styling}" href="/${slug}.html" data-page-name="${slug}" title="${resolve.resolutionContext.join(' => ')}">${escape(name)}</a>`,
      )
    } else {
      return match
    }
