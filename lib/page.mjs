...
import { create as revision_create , apply as revision_apply } from './revision.mjs';
import synopsis from './synopsis.cjs';
...
var newPage = function(json, site) {
  ...
  const addItem = function(item) {
    item = Object.assign({}, {id: itemId()}, item);
  ...
  const merge = function(update) {
    ...
    newPage(revision_create(999, {title: page.title, journal: merged}), site);
  };

  const apply = function(action) {
    revision_apply(page, action);
...
export {newPage, asSlug, asTitle, pageEmitter};
