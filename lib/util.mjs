...
export function formatDate(msSinceEpoch) {
...
export function formatActionTitle(action) {
  let title = '';
  if (action.site != null) { title += `${action.site}\n`; }
  title += action.type || 'separator';
  if (action.date != null) { title += ` ${formatElapsedTime(action.date)}`; }
  if ((action.attribution != null ? action.attribution.page : undefined) != null) { title += `\nfrom ${action.attribution.page}`; }
  if ((action.removedTo != null ? action.removedTo.page : undefined) != null) { title += `\nto ${action.removedTo.page}`; }
  return title;
};

export function formatElapsedTime(msSinceEpoch) {
...
export function formatDelay(msSinceEpoch) {
  let hrs, mins, secs;
