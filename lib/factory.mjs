...
import { createSynopsis } from './synopsis.mjs';
...
export function bind ($item, item) {
  ...
  const syncEditAction = function() {
    ...
    return pageHandler.put($page, {type: 'edit', id: item.id, item});
  };
  ...
  const addReference = data => wiki.site(data.site).get(`${data.slug}.json`, function(err, remote) {
    if (!err) {
      ...
      item.text = createSynopsis(remote);
      syncEditAction();
      if (item.site != null) { return neighborhood.registerNeighbor(item.site); }
    }
  });
  ...
  const addRemoteImage = function(url) {
    ...
    return fetch(url)
      .then(function(response) {
        ...
        throw new Error('Unable to fetch image');}).then(function(imageBlob) {
        ...
        return reader.onload = function(loadEvent) {
          ...
          return window.plugins['image'].editor({ imageDataURL, filename: imageFileName, imageSourceURL: url, imageCaption: `Remote image [${url} source]`, $item, item });
        };
    });
  };
  ...
  const addRemoteSvg = function(url) {
    ...
    .then(function(response) {
      ...
      throw new Error('Unable to fetch svg');}).then(response => response.text()).then(function(svgText) {
  ...
  const readFile = function(file) {
    if (file != null) {
      const [majorType, minorType] = Array.from(file.type.split("/"));
      const reader = new FileReader();
      if (majorType === "image") {
        // svg -> html plugin
        if (minorType.startsWith('svg')) {
          ...
          reader.onload = function(loadEvent) {
            ...
            const imageDataURL = loadEvent.target.result;
            return window.plugins['image'].editor({ imageDataURL, filename: file.name, imageCaption: "Uploaded image" , $item, item});
          };
